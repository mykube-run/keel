...
// Connect handles the bidirectional gRPC stream initiated by a worker.
// Protocol expectations:
//   - The first meaningful envelope from the worker must include a non-empty `From` field,
//     which is used as the worker identifier and the key under which the stream is registered.
//   - Once registered, every incoming envelope is forwarded to the transport's message handler (`omr`),
//     if present. Any handler error is logged together with the handler's returned bytes for easier debugging.
//   - If the transport is closing (`closeReceiving`), the stream is terminated gracefully.
//
// Error handling:
// - A receive error (`stream.Recv`) is considered terminal for this stream and is logged before returning.
// Concurrency:
// - Registration of the worker stream is protected by `streamsMu` to maintain the integrity of `workerStreams`.
func (s *transportServer) Connect(stream pb.Transport_ConnectServer) error {
	// Lazily set once the first valid envelope provides the worker identity.
	var workerId string
	for {
		// Exit if the transport has been instructed to stop receiving.
		if s.t.closeReceiving {
			return nil
		}
		// Block until an envelope arrives from the worker.
		env, err := stream.Recv()
		if err != nil {
		...
		// On the first envelope that includes a non-empty `From`, register the stream
		// so scheduler-side send operations can target this worker.
		...
		// Forward the message to the transport's on-message-received callback, if configured.
		// Log any handler error together with the returned bytes to aid troubleshooting.
		if s.t.omr != nil {
